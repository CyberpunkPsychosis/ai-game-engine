...
- **双态结构（2026-06 定稿）**：残响默认**沉眠**(只重演死前动作→解谜阶段,冻结=带时机的踏板谜题)；区域深处**「醒钟」**玩家自选敲否——敲=怪苏醒爽快开战+完整奖励,不敲=潜行过但奖励大减(能量靠打攒→自balance)。每房的选择=终局"重启时间 vs 凝滞之主"的微缩预演。
- 下一步候选：**实现双态怪物(沉眠循环态)+醒钟**；把"冻敌当踏板"做成必须冻才能过的谜题（竖井/高台）；F1 实时调参接入。
...
- **分工（2026-06 定，最终版）**：**素材生产=用户亲自做**（手动操作各 AI 平台/自学像素画，用户实测自己动手效果更好）；**Claude 只负责两件事：① 接收用户给的现成素材**（切帧/抠图收尾/入库/接引擎，素材放 `incoming/` 或直接发）**② Godot 游戏开发本身**（玩法/手感/关卡/系统）。Claude **不再主动调 AI 生成素材、不替用户抽卡**。
  - **用户的素材生产手册**：角色=`docs/CHAR_PIPELINE_3D.md`（3D 中转：捏人/绑骨/动作库/Retarget/渲染/像素化）；瓦片/地图=`docs/TILESET_PIPELINE.md`（AI 瓦片/现成包/Tilesetter 组装，配 `TILE_ASSETS.md` 清单用）；调研历史结论见 `docs/AI_TOOLS.md` 末尾。
...
- `tools/enemy-designer/` 怪物路线画布 · `tools/composer/` 关卡 · `tools/tilemap/` 瓦片（通用编辑器，当前 TimeStop 未必都接；用前喊 Claude 焊对接管道）
