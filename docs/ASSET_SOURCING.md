# 素材搜集指南 · 2D 像素弹反 Roguelite

> 怎么找、搜什么词、挑什么、授权怎么看、怎么交给引擎侧。
> 配合 `CAST.md`(要哪些角色)、`ENEMY_DESIGN.md`(要前摇/攻击帧)、`CLOUD_WORKFLOW.md`(AI 出图)。
...
3. 我:`char_intake`/切片入库 → 接角色控制器/敌人 AI(前摇→命中帧→弹反判定)→ tileset 接关卡。
