...
## 一句话定位
**"沉眠区解谜潜行 → 敲醒钟自选开战 → 实时快打 + 随手冻结搞事 + 绝境全场定格翻盘"** 的横版动作游戏。
**双态结构(2026-06 定稿)**:残响默认「沉眠」(只重演死前动作,解谜阶段,冻结=带时机的踏板/开路工具)
→ 区域深处「醒钟」由玩家选择敲否 → 敲=残响苏醒(完整战斗 AI)+完整奖励;不敲=潜行过但奖励大减
(能量靠打架攒 → 不打自然资源饥荒,潜行流=高技巧低资源)。设定详见 `WORLD.md` 双态结构节。
...
- **敌人三型**：冲锋 charger / 远程弹幕 shooter / 治疗 healer。**双态**:沉眠(只循环死前动作,新增,待实现)/苏醒(下述完整 FSM)。**通用 AI 走标准平台怪 FSM**(参考死亡细胞):
  `patrol`(以出生点为中心 ±220 固定来回, 撞墙/崖边/边界掉头, 不掉崖, **不主动追**)→ `alert`(进 360px 察觉范围才警觉)
  → `chase`(追, 崖边停不跳崖)→ `windup`(预警闪红框)→ `lunge`(锁向扑杀、冲过头, 仅这下碰到伤人)→ `recover`(破绽)。
  **被打 = hitstun**:击退 + **取消当前动作/扑击** + 短硬直(`enemy.stagger()`, `game._hit_enemy` 调)。
  shooter 进战才保持中距开火、healer 见你就逃, 都不掉崖。(后续用 enemy-designer 给每种怪重做细节。)
...
- **平台手感四件套**（`player.gd`, 2026-06 加, 业界标配）：
  - **土狼时间**(coyote 0.14s)：离开平台边缘后短暂仍可跳, 不会一脚踏空。
    (⚠坑:coyote 只在**空中**才流逝、离台当帧不扣——否则离开瞬间先被扣一帧, 实际窗口只剩一半。)
  - **跳跃缓冲**(buffer 0.12s)：落地前提前按跳, 落地瞬间自动起跳。
  - **变量跳跃高度**：按住跳得高、轻点跳得矮(松手截断上升 `JUMP_CUT`);手机靠按住"跳"键时长决定。
  - **非对称重力**：接近顶点减重(留空感) + 下落加重(利落不飘)。
  - **抓沿/攀爬(ledge grab)**：跳到平台边缘够到沿口→抓住挂住(停重力);**按跳=翻身爬上**、
    **往沿外推=松手**、不操作挂 0.30s 自动爬上。判定:面朝方向前方实体的顶沿落在头部"够得到"
    带里 + 沿口下面是墙/上面是空(`player.gd: _ledge_in_front`)。
  - 附:起跳拉伸 / 落地压扁(`_squash`)做手感反馈。
