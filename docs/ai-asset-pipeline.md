...
## 7. 【硬性规则】视频生成内容严禁出画 — 长刀斩事故复盘

> **规则(用户钦定): 以后让模型出视频, 一定不能有任何内容在画面外。**
> 角色身体、手持武器、飘动衣发全算"内容"; "刀光梢触边可接受"的说法已被实战
> 推翻(80px 下亮色刀身切平肉眼可见, 且抠图链路会放大缺损)。
>
> 三道强制保险, 每次 i2v 生成缺一不可:
> 1. **垫画布**: 参考图先贴到更大纯色画布中央(角色 ≤60% 画面高, 四周余量 ≥20%)
>    再喂模型 — 这是主保险, 零成本。
> 2. **prompt 写明**: "the entire character and her weapon stay fully visible
>    inside the frame at all times, wide margins around the character"。
> 3. **生成后触边自检**: 背景差分统计每帧画框四边前景像素, 任何帧身体或武器
>    触边 → 该次生成判废重抽, 不许"挑着用"。
