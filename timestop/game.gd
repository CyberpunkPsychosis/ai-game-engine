...
var _cam_pos := Vector2.ZERO     # 手动分轴跟随的相机位置(HK 式: 横快纵慢)
var _cam_anchor_y := 0.0         # 纵轴锚=最近站立的地面高度(跳跃不带镜头)
...
func _build_camera() -> void:
	...
	cam.position_smoothing_enabled = false   # HK 式分轴跟随在 _process 里手动算(内置平滑不分轴)
	cam.limit_left = 0
	cam.limit_top = 0
	cam.limit_right = int(room_w)
	cam.limit_bottom = int(room_h)
...
func _process(delta: float) -> void:
	...
	if cam:                              # HK 式分轴跟随(limit 自动夹边), 前瞻+震屏走 offset
		# 纵轴锚定地面: 站着锚=脚下地面; 空中锚不动(跳跃不带镜头),
		# 仅当人要出"视窗"(上130/下90)才被拖走 → 落差大时镜头才跟落。
		if player.onground:
			_cam_anchor_y = player.position.y
		else:
