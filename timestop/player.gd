...
# 动作版疾跑 8 帧 1x 显示实测蹬地≈120px/s;步频按 |vx|/RUN_TREAD 缩放消除滑步感。
# 疾跑动画可承受完全咬合(全速 2.7x 步频≈4步/秒, 正是动作游戏冲刺节奏)。
const RUN_TREAD := 120.0
...
func tick(delta: float) -> void:
	...
	if _use_sprite and _anim and _anim.sprite_frames:
		...
		_anim.speed_scale = clampf(absf(vx) / RUN_TREAD, 0.5, 3.0) if a == "run" else 1.0
	queue_redraw()
